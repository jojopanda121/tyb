...
class PeerInfoCrew:
    """
    目的：承接同行筛选与同行数据底稿专题的 research 子 crew。
    功能：围绕同行集合、peer 财务与估值倍数产出 peer_info 分析包。
    实现逻辑：本文件直接声明同行专题的 agent、task 和 crew，不再依赖共享基类。
    可调参数：YAML 配置、专题 guidance、额外工具工厂、日志路径和模型温度。
    默认参数及原因：默认注入同行估值工具，原因是当前专题的目标就是给 valuation 阶段准备底稿。
    """
    ...
    extra_tool_factories: tuple[Callable[[], object], ...] = (TushareValuationDataTool, ComparableValuationTool)
    ...
    def _extra_tools(self) -> list[object]:
        ...
        默认参数及原因：默认注入 Tushare 与可比估值工具，原因是同行专题本身就要准备估值底稿。
        """
    ...
    def _search_tools(self) -> list[object]:
        ...
        功能：把搜索、registry 写入和同行专题额外工具按固定顺序注入给外部搜索 agent。
        实现逻辑：先按需加搜索工具，再追加 registry 和专题扩展工具。
        可调参数：`use_search_tool` 和 `extra_tool_factories`。
        默认参数及原因：默认启用 `SerperDevTool`，原因是同行筛选高度依赖外部公开资料。
        """
    ...
    def _extract_tools(self) -> list[object]:
        ...
        功能：把 PDF 读取、registry 写入和同行专题额外工具按固定顺序注入给原文提取 agent。
    ...
    def search_fact_agent(self) -> Agent:
        """
        目的：定义同行专题的外部搜索 agent。
        功能：补充同行筛选、主营差异和可比性限制相关公开资料，并回写 registry。
        实现逻辑：使用同行专题的搜索工具集与通用 research 运行参数构建 Agent。
        可调参数：YAML agent 配置、搜索工具和模型温度。
        默认参数及原因：默认沿用专题基础温度，原因是同行搜索需要兼顾广度和稳定性。
        """
    ...
    def extract_file_fact_agent(self) -> Agent:
        """
        目的：定义同行专题的原文提取 agent。
        功能：回到 PDF 原文提取管理层自认可比对象和竞争对手线索，并回写 registry。
        实现逻辑：使用 PDF + registry 工具集与通用 research 运行参数构建 Agent。
        可调参数：YAML agent 配置、PDF 工具和模型温度。
        默认参数及原因：默认沿用专题基础温度，原因是同行原文提取以稳定取证为先。
        """
    ...
    def synthesizing_agent(self) -> Agent:
        """
        目的：定义同行专题的综合输出 agent。
        功能：把已沉淀的同行事实、数据、判断和冲突整理成 Markdown 分析包。
    ...
    def search_facts(self) -> Task:
        """
        目的：定义同行专题的外部搜索任务。
        功能：驱动搜索 agent 补足同行公开资料、证据和 judgment。
    ...
    def check_registry(self) -> Task:
        """
        目的：定义同行专题的内部 QA 任务。
        功能：驱动 QA agent 检查 peer_info pack 的账本闭环情况。
    ...
    def synthesize_and_output(self) -> Task:
        """
        目的：定义同行专题的最终综合输出任务。
        功能：驱动综合 agent 输出最终 peer_info pack Markdown。
    ...
    def crew(self) -> Crew:
        """
        目的：输出同行专题最终使用的层级 research crew。
        ...
        默认参数及原因：默认采用 `Process.hierarchical`，原因是同行专题仍按设计由 manager 统一调度。
        """
