...
DEFAULT_CREW_LOG_FILE = str(PROJECT_LOG_DIR / "industry_crew.json")
...
class IndustryCrew:
    """
    目的：承接行业专题的 research 子 crew。
    功能：围绕行业定义、增长驱动、竞争格局和监管环境产出行业分析包。
    实现逻辑：本文件直接声明行业专题的 agent、task 和 crew，不再依赖共享基类。
    可调参数：YAML 配置、专题 guidance、额外工具工厂、日志路径和模型温度。
    默认参数及原因：默认更强调外部搜索，原因是行业数据和竞争格局通常不在单一 PDF 中完整给出。
    """
    ...
    def _extra_tools(self) -> list[object]:
        ...
        默认参数及原因：默认返回空列表，原因是行业专题当前只依赖通用 research tools。
        """
    ...
    def _search_tools(self) -> list[object]:
        ...
        默认参数及原因：默认启用 `SerperDevTool`，原因是行业专题高度依赖外部公开资料。
        """
    ...
    def _extract_tools(self) -> list[object]:
        ...
        可调参数：`extra_tool_factories`。
        默认参数及原因：默认总是注入 PDF 工具，原因是行业专题也必须回到原始披露材料取证。
        """
    ...
    def search_fact_agent(self) -> Agent:
        """
        目的：定义行业专题的外部搜索 agent。
        功能：补充行业数据、竞争格局和监管信息，并回写 registry。
        实现逻辑：使用行业专题的搜索工具集与通用 research 运行参数构建 Agent。
        可调参数：YAML agent 配置、搜索工具和模型温度。
        默认参数及原因：默认沿用专题基础温度，原因是行业搜索需要在广度和稳定性之间平衡。
        """
    ...
    def extract_file_fact_agent(self) -> Agent:
        """
        目的：定义行业专题的原文提取 agent。
        功能：回到 PDF 原文提取行业定义、定位和管理层口径，并回写 registry。
        实现逻辑：使用 PDF + registry 工具集与通用 research 运行参数构建 Agent。
        可调参数：YAML agent 配置、PDF 工具和模型温度。
        默认参数及原因：默认沿用专题基础温度，原因是行业原文提取以稳定取证为先。
        """
    ...
    def qa_check_agent(self) -> Agent:
        """
        目的：定义行业专题的内部 QA agent。
        功能：检查当前 industry pack 的 registry 覆盖度，并补 revision_detail 与 review 留痕。
    ...
    def synthesizing_agent(self) -> Agent:
        """
        目的：定义行业专题的综合输出 agent。
        功能：把已沉淀的行业事实、数据、判断和冲突整理成 Markdown 分析包。
    ...
    def search_facts(self) -> Task:
        """
        目的：定义行业专题的外部搜索任务。
        功能：驱动搜索 agent 补足行业公开资料、证据和 judgment。
    ...
    def check_registry(self) -> Task:
        """
        目的：定义行业专题的内部 QA 任务。
        功能：驱动 QA agent 检查 industry pack 的账本闭环情况。
    ...
    def synthesize_and_output(self) -> Task:
        """
        目的：定义行业专题的最终综合输出任务。
        功能：驱动综合 agent 输出最终 industry pack Markdown。
    ...
    def crew(self) -> Crew:
        """
        目的：输出行业专题最终使用的层级 research crew。
        ...
        默认参数及原因：默认采用 `Process.hierarchical`，原因是行业专题仍按设计由 manager 统一调度。
        """
