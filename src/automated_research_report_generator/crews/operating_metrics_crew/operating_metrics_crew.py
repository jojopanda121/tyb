...
class OperatingMetricsCrew:
    """
    目的：承接运营指标专题的 research 子 crew。
    功能：围绕订单、产能、利用率、客户数等关键运营指标产出运营指标分析包。
    实现逻辑：本文件直接声明运营指标专题的 agent、task 和 crew，不再依赖共享基类。
    可调参数：YAML 配置、专题 guidance、额外工具工厂、日志路径和模型温度。
    默认参数及原因：默认保持搜索为主，原因是运营指标常常分散在公告、路演和行业资料里。
    """
    ...
    def _extra_tools(self) -> list[object]:
        ...
        默认参数及原因：默认返回空列表，原因是运营指标专题当前只依赖通用 research tools。
        """
    ...
    def _search_tools(self) -> list[object]:
        ...
        默认参数及原因：默认启用 `SerperDevTool`，原因是运营指标专题高度依赖外部公开资料。
        """
    ...
    def _extract_tools(self) -> list[object]:
        ...
        可调参数：`extra_tool_factories`。
        默认参数及原因：默认总是注入 PDF 工具，原因是运营指标专题仍需回到原文取证。
        """
    ...
    def manager_agent(self) -> Agent:
        """
        目的：定义运营指标专题的层级调度 manager agent。
    ...
    def search_fact_agent(self) -> Agent:
        """
        目的：定义运营指标专题的外部搜索 agent。
        功能：补充订单、产能、利用率等公开资料，并回写 registry。
        实现逻辑：使用运营指标专题的搜索工具集与通用 research 运行参数构建 Agent。
        可调参数：YAML agent 配置、搜索工具和模型温度。
        默认参数及原因：默认沿用专题基础温度，原因是运营指标搜索需要在广度和稳定性之间平衡。
        """
    ...
    def extract_file_fact_agent(self) -> Agent:
        """
        目的：定义运营指标专题的原文提取 agent。
        功能：回到 PDF 原文提取运营指标线索，并回写 registry。
        实现逻辑：使用 PDF + registry 工具集与通用 research 运行参数构建 Agent。
        可调参数：YAML agent 配置、PDF 工具和模型温度。
        默认参数及原因：默认沿用专题基础温度，原因是运营指标原文提取以稳定取证为先。
        """
    ...
    def qa_check_agent(self) -> Agent:
        """
        目的：定义运营指标专题的内部 QA agent。
        功能：检查当前 operating_metrics pack 的 registry 覆盖度，并补 revision_detail 与 review 留痕。
    ...
    def synthesizing_agent(self) -> Agent:
        """
        目的：定义运营指标专题的综合输出 agent。
        功能：把已沉淀的运营指标事实、数据、判断和冲突整理成 Markdown 分析包。
    ...
    def search_facts(self) -> Task:
        """
        目的：定义运营指标专题的外部搜索任务。
        功能：驱动搜索 agent 补足运营指标公开资料、证据和 judgment。
    ...
    def check_registry(self) -> Task:
        """
        目的：定义运营指标专题的内部 QA 任务。
        功能：驱动 QA agent 检查 operating_metrics pack 的账本闭环情况。
    ...
    def synthesize_and_output(self) -> Task:
        """
        目的：定义运营指标专题的最终综合输出任务。
        功能：驱动综合 agent 输出最终 operating_metrics pack Markdown。
    ...
    def crew(self) -> Crew:
        """
        目的：输出运营指标专题最终使用的层级 research crew。
        ...
        默认参数及原因：默认采用 `Process.hierarchical`，原因是运营指标专题仍按设计由 manager 统一调度。
        """
