...
class FinancialCrew:
    ...
    def _search_tools(self) -> list[object]:
        ...
        功能：把搜索、registry 写入和财务专题额外工具按固定顺序注入给外部搜索 agent。
        实现逻辑：先按需加搜索工具，再追加 registry 和专题扩展工具。
        可调参数：`use_search_tool` 和 `extra_tool_factories`。
        默认参数及原因：默认仍保留 `SerperDevTool`，原因是财务专题偶尔需要补缺失公开数据。
        """
    ...
    def _extract_tools(self) -> list[object]:
        ...
        功能：把 PDF 读取、registry 写入和财务专题额外工具按固定顺序注入给原文提取 agent。
        实现逻辑：先放页索引和页内容工具，再追加 registry 和专题扩展工具。
        可调参数：`extra_tool_factories`。
        默认参数及原因：默认总是注入 PDF 工具，原因是财务专题必须优先回到原始披露材料取证。
        """
    ...
    def search_fact_agent(self) -> Agent:
        """
        目的：定义财务专题的外部搜索 agent。
        功能：补充缺失财务公开资料，并回写 registry。
        实现逻辑：使用财务专题的搜索工具集与通用 research 运行参数构建 Agent。
    ...
    def extract_file_fact_agent(self) -> Agent:
        """
        目的：定义财务专题的原文提取 agent。
        功能：回到 PDF 原文提取报表、附注和现金流线索，并回写 registry。
    ...
    def qa_check_agent(self) -> Agent:
        """
        目的：定义财务专题的内部 QA agent。
        功能：检查当前 finance pack 的 registry 覆盖度，并补 revision_detail 与 review 留痕。
    ...
    def synthesizing_agent(self) -> Agent:
        """
        目的：定义财务专题的综合输出 agent。
        功能：把已沉淀的财务事实、数据、判断和冲突整理成 Markdown 分析包。
    ...
    def search_facts(self) -> Task:
        """
        目的：定义财务专题的外部搜索任务。
        功能：驱动搜索 agent 补足财务公开资料、证据和 judgment。
    ...
    def check_registry(self) -> Task:
        """
        目的：定义财务专题的内部 QA 任务。
        功能：驱动 QA agent 检查 finance pack 的账本闭环情况。
    ...
    def synthesize_and_output(self) -> Task:
        """
        目的：定义财务专题的最终综合输出任务。
        功能：驱动综合 agent 输出最终 finance pack Markdown。
