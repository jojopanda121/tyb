...
- research QA 不再以 `question_tree` 为覆盖基准，改为以 registry 中 `high + medium` 优先级条目为阻塞基准；`low` 只记录缺口，不阻塞主流程。
...
- 模板体系：
  ...
  - YAML 文件使用，显式写出稳定 `entry_id`，包含这些字段：
    ...
    - `owner_crew`: 负责回答该问题的crew，`business_crew`/`financial_crew`/`history_background_crew`/`industry_crew`/`operating_metrics_crew`/ `peer_info_crew`
    - `priority` : high/medium/low
    - `title`: 关注问题
    - `description`: 关注问题详细描述及对输出预期的指引
    - `content`: 必填，agent填写答案，可以是数字、文字或数字及文字的结合，可以是无信息
    - `unit` : 必填，agent填写答案，具体单位/不适用，可以是无信息
    - `period` : 必填，agent填写答案，具体年份/年月日/也可以不适用，可以是无信息
    - `source` : 必填，website url / inputfile_page_1 / inputfile_page_50-52 / tushare / ....，可以是不适用
    - `status` ： unchecked（待解答）/ checked（已解答）
...
- Crew对Registry的使用方法：
  ...
  - 在输入模型中加入筛选与缩放参数：
   ...
   - 7 个 research crews 的 prompt 改成先按 `owner_crew` + `status` 读取自己的 entry，再补证、补状态。
   - research里的每个crew里`extract_file_fact_agent`和`extract_file_fact_agent` 可以以 `entry_id` or `owner_crew` or `status` 读取自己的 entry，再补证、补状态
   - research里的每个crew里`synthesizing_agent` 按 `owner_crew` 读取自己的 entry，再综合输出。
   - research里的每个crew里`qa_agent` 按 `status` 读取自己的 entry，再提示mananger将相关续补证`entry_id`返回`extract_file_fact_agent`和`extract_file_fact_agent` 。
   - 后续的valuation_crew、investment thesis_crew在读取registry的时候，需给他们返回整个registry `title` `descrption`
...
- registry 模板成为新的人工维护接口：7 个 pack YAML 文件就是 planning phase 的唯一“研究引导源”。
- research / QA / writeup 输入中移除 `research_scope_text` 与 `question_tree_text`。

## Test Plan
- 模板加载：
  - 成功加载 7 个 pack YAML，并生成 registry 快照
  - 重复 ID、pack 不匹配、非法 owner、缺字段时明确失败
  - `{company_name}` / `{industry}` 插值正确，UTF-8 中文不损坏
- Flow：
